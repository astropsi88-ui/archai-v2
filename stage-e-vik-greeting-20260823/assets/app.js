...
function initVikVoicePrototype() {
  ...
  const publicVoiceGreeting = "Здравствуйте! Я Вик, AI-консультант ARCH_AI. Расскажите немного о вашем бизнесе и задаче — я помогу подобрать подходящего AI-сотрудника или команду";
  ...
  const startElevenLabsVoice = async (suppliedToken = <redacted> => {
    ...
    const startMicAfterGreeting = async () => {
      if (!state.greetingPhase || state.micStarting) return;
      if (!state.greetingTextConfirmed) throw new Error("elevenlabs_greeting_mismatch");
      state.micStarting = true;
      const waitMs = Math.max(0, Math.ceil((state.nextAudioAt - outputContext.currentTime) * 1000) + 30);
      if (waitMs) await new Promise((resolve) => setTimeout(resolve, waitMs));
      if (active !== state || outputContext.state === "closed") return;
      ...
      processor.onaudioprocess = (event) => {
        if (ws.readyState !== WebSocket.OPEN || state.greetingPhase) return;
      ...
      Object.assign(state, { stream, inputContext, processor, micSource, silent, greetingPhase: false });
    ...
    ws.addEventListener("message", (event) => {
      ...
      if (data.type === "conversation_initiation_metadata") setStatus("Вик приветствует вас");
      ...
      if (data.type === "agent_response") {
        ...
        if (reply) {
          if (state.greetingPhase) {
            state.greetingTextConfirmed = reply === publicVoiceGreeting;
            if (!state.greetingTextConfirmed) {
              state.pendingGreetingAudio = [];
              return;
            }
            const greetingAudio = state.pendingGreetingAudio.splice(0);
            greetingAudio.forEach(playPcm16);
            return;
          }
      ...
      if (data.type === "audio") {
        ...
        if (audio && state.greetingPhase && !state.greetingTextConfirmed) state.pendingGreetingAudio.push(audio);
        else if (audio) playPcm16(audio);
      }
      if (data.type === "agent_response_end" || data.type === "agent_response_complete") {
        if (state.greetingPhase) {
          void startMicAfterGreeting().catch((error) => {
            console.info("vik_elevenlabs_greeting_error", { code: error.message });
            closeSession();
          });
          return;
        }
    ...
    await new Promise((resolve, reject) => {
      ...
      ws.addEventListener("open", () => {
        ...
        ws.send(JSON.stringify({
          ...
          conversation_config_override: { agent: { first_message: publicVoiceGreeting } },
        }));
        resolve();
      }, { once: true });
    ...
    setStatus("Вик приветствует вас");
  };
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      setStatus("Подключаю голос Вика");
      ...
      if (usePreferredVoiceEngine) {
        ...
        if (preferredToken.mode === "elevenlabs_speech_engine") {
          await startElevenLabsVoice(preferredToken);
          return;
        }
      }
      const stream = await navigator.mediaDevices.getUserMedia({
